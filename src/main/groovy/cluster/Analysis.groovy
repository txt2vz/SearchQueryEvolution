...
class Analysis {
    ...
    void reportsOut(int jobNumber, int gen, int popSize, int numberOfSubpops, int genomeSizePop0, int maxGenePop0, ECJclusterFitness cfit, QType qt, t3ClassiferResult = null, LuceneClassifyMethod classifyMethod = null, boolean onlyDocsInOneCluster) {
        ...
        if (t3ClassiferResult) {
            ...
            GAplusLucene << "${averageF1ForJob.round(5)}, ${averagePrecisionForJob.round(5)}, ${averageRecallForJoab.round(5)}, ${cfit.getFitness().round(5)}, ${t3ClassiferResult.first}, ${t3ClassiferResult.second}, ${t3ClassiferResult.third},  " +
                    " ${classifyMethod.name()},  ${Indexes.index.name()}, ${qt}, ${ECJclusterFitness.k_PENALTY}, $onlyDocsInOneCluster, $numberOfClusters, $numberOfOriginalClasses, $categoryCountError, $categoryCountErrorAbs, $gen, $jobNumber , ${new Date()} \n"
        }
        ...
        fcsv << "${averageF1ForJob.round(5)}, ${averagePrecisionForJob.round(5)}, ${averageRecallForJoab.round(5)}, ${cfit.getFitness().round(5)}, ${Indexes.index.name()}, ${cfit.FITNESS_METHOD}, ${ECJclusterFitness.k_PENALTY}, $numberOfSubpops, $popSize, $genomeSizePop0, $maxGenePop0, " +
                "${ClusterQueryECJ.QUERY_TYPE}, ${QueryListFromChromosome.intersectMethod.intersectRatio}, $numberOfClusters, $numberOfOriginalClasses, $categoryCountError, $categoryCountErrorAbs, $gen, $jobNumber , ${new Date()} \n"

        Tuple5 indexAndParams = new Tuple5(Indexes.index.name(), ECJclusterFitness.FITNESS_METHOD, ClusterQueryECJ.QUERY_TYPE, QueryListFromChromosome.intersectMethod, jobNumber)
